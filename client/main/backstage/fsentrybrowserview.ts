...
export const FSEntryBrowserView = SilkyView.extend({
  initialize: function () {
    ...
    if (!this.model) this.model = new FSEntryListModel();

    this.model.on(
      "change:items change:dirInfo change:status",
      this._render,
      this
    );
  ...
  _suggestedChanged: function (event) {
    ...
    if ($saveName.length > 0) {
      ...
      if (filePath) $button.removeClass("disabled-div");
      else $button.addClass("disabled-div");
    }
  },
  _listPointerDown: function (event) {
    ...
    if (this._clickedItem) {
      let fromChecked =
        event.target.classList.contains("jmv-bs-fslist-checkbox") &&
  ...
  _listFocus: function (event) {
    ...
    if (selectedIndex >= 0 && selectedIndex < this.$items.length) {
      ...
      this.$items[selectedIndex]
        .find(".jmv-bs-fslist-checkbox")
        .prop("checked", true);
      this._selected = true;

      let offset = this.$items[selectedIndex].position();
      if (offset.top < 0)
        this.$itemsList.animate(
          { scrollTop: this.$itemsList.scrollTop() + offset.top },
          100
        );
  ...
  _setSelection: function ($target) {
    if (this._selectedIndices.length > 0) this.clearSelection();
  ...
  _orderItems: function (orderby, direction, items) {
    ...
    if (orderby === "type") {
      for (let i = 0; i < items.length - 1; i++) {
        ...
        let item2 = items[i + 1];
        if (
          (direction === 0 && item1[orderby] > item2[orderby]) ||
          (direction === 1 && item1[orderby] < item2[orderby])
        ) {
          items[i] = item2;
          items[i + 1] = item1;
          if (i > 1) i -= 2;
  ...
  _manualBrowse: function (event) {
    ...
    let type = "open";
    if (
      this.model.clickProcess === "save" ||
      this.model.clickProcess === "export"
    ) {
      type = "save";
      filename = this.$header
        .find(".silky-bs-fslist-browser-save-name")
        .val()
        .trim();
    } else if (this.model.clickProcess === "import") type = "import";

    this.model.requestBrowse({
      list: this.model.fileExtensions,
      type,
      filename,
    });
  },
  _createFileTypeSelector: function () {
    ...
    html += `               <select class="silky-bs-fslist-browser-save-filetype-inner" aria-label="${_(
      "File type"
    )}">`;
    for (let i = 0; i < this.model.fileExtensions.length; i++) {
      let exts = this.model.fileExtensions[i].extensions;
      let desc =
        this.model.fileExtensions[i].description === undefined
          ? this.model.fileExtensions[i].name
          : this.model.fileExtensions[i].description;
      let selected = "";
  ...
  _createFooter: function () {
    ...
    if (isSaving === false) {
      ...
      if (multiSelect) {
        html +=
          '   <div class="silky-bs-fslist-import-options hidden" style="display: flex; flex-flow: row nowrap;">';
        html += `       <input class="silky-bs-fslist-browser-import-name" type="text" placeholder="${_(
          "Enter file name here"
        )}" />`;
        html +=
          '       <div class="silky-bs-fslist-browser-import-button" style="display: flex; flex: 0 0 auto;">';
  ...
  _createBody() {
    ...
    if (!isSaving) {
      let searchHtml = `<div class="searchbox">
                            <div class="image"></div>
                            <input class="search" type="search" aria-label="${_(
                              "Search files"
                            )}" aria-controls="${itemListId}"></input>
                        </div>`;
      this.$el.append(searchHtml);

      let $search = this.$el.find(".searchbox > .search");
      if ($search.length > 0)
        focusLoop.applyShortcutOptions($search[0], {
          key: "S",
          position: { x: "5%", y: "50%" },
        });
    }
    ...
    this.$itemsList = $(
      `<div id="${itemListId}" role="list" aria-multiselectable="false" class="silky-bs-fslist-items" style="flex: 1 1 auto; overflow: auto; height:100%" tabindex="0"></div>`
    );
  ...
  _createHeader() {
    ...
    if (isSaving) {
      html +=
        '   <div class="silky-bs-fslist-save-options" style="display: flex; flex-flow: row nowrap;">';
      ...
      html += `           <input class="silky-bs-fslist-browser-save-name" type="text" placeholder="${_(
        "Enter file name here"
      )}" ${insert} />`;
      ...
      html += "       </div>";
      html +=
        '       <button class="silky-bs-fslist-browser-save-button' +
        s6e(filePath ? "" : " disabled-div") +
    ...
    if (!this.model.writeOnly) {
      ...
      if (this.model.get("multiselect"))
        html +=
          '       <button class="silky-bs-fslist-browser-check-button"></button>';
      html += `       <button class="silky-bs-fslist-browser-back-button" aria-label="${_(
        "Move back directory"
      )}"><span class="mif-arrow-up"></span></button>`;
      html +=
        '       <div class="silky-bs-fslist-browser-location" style="flex: 1 1 auto;"></div>';

      if (this.model.attributes.browseable) {
        html += '       <button class="silky-bs-fslist-browse-button">';
        html +=
          '           <div class="silky-bs-fslist-browser-location-icon silky-bs-flist-item-folder-browse-icon"></div>';
    ...
    if ($multiMode.length > 0)
      focusLoop.applyShortcutOptions($multiMode[0], {
        key: "C",
        action: this._toggleMultiMode.bind(this),
      });

    let $back = this.$header.find(".silky-bs-fslist-browser-back-button");
    if ($back.length > 0)
      focusLoop.applyShortcutOptions($back[0], {
        key: "B",
        position: { x: "20%", y: "25%" },
        action: this._backClicked.bind(this),
        blocking: true,
      });

    let $browse = this.$header.find(".silky-bs-fslist-browse-button");
    if ($browse.length > 0)
      focusLoop.applyShortcutOptions($browse[0], {
        key: "E",
        action: this._manualBrowse.bind(this),
      });
    ...
    if (
      focusLoop.inAccessibilityMode() === false &&
      (this.model.clickProcess === "save" ||
        this.model.clickProcess === "export")
    ) {
      setTimeout(() => {
    ...
    if (this.model.attributes.extensions) {
      ...
      if (extValue != -1)
        this.$el
          .find(".silky-bs-fslist-browser-save-filetype-inner")
          .val(extValue);
    }
  },
  ...
  _render: function () {
    ...
    $search.attr(
      "aria-label",
      _("Search for file in directory {0}", [filePath])
    );
    ...
    if ($backbutton)
      $backbutton.attr(
        "aria-label",
        _("Move back from directory {0}", [filePath])
      );
    ...
    if (status === "error") {
      ...
      if (searchValue !== "") {
        if (isUrl(searchValue)) {
          try {
            ...
            let description = _("An online data set hosted by {hostname}", {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (!item.isUrl && searchValue !== "") {
          ...
          if (lname.includes(lsearchValue) === false) {
            if (
              !item.description ||
              item.description.toLowerCase().includes(lsearchValue) === false
            ) {
              let found = false;
        ...
        if (itemType === FSItemType.File)
          html +=
            '<input class="jmv-bs-fslist-checkbox' +
            (this.multiMode ? "" : " hidden") +
            '" type="checkbox" tabindex="-1">';
        html += '   <div class="silky-bs-flist-item-icon">';
        if (itemType === FSItemType.File) {
          //file
          if (item.isExample)
            // examples don't have extensions
            html +=
              '       <div class="silky-bs-flist-icon silky-bs-flist-item-csv-icon"></div>';
          else if (lname.endsWith(".omv"))
            html +=
              '       <div class="silky-bs-flist-icon silky-bs-flist-item-omv-icon"></div>';
          else if (lname.endsWith(".omt"))
            html +=
              '       <div class="silky-bs-flist-icon silky-bs-flist-item-omt-icon"></div>';
            ...
            html +=
              '       <div class="silky-bs-flist-icon silky-bs-flist-item-csv-icon"></div>';
        } else if (itemType === FSItemType.Folder)
          //folder
          html +=
            '       <div class="silky-bs-flist-icon silky-bs-flist-item-folder-icon"></div>';
        else if (itemType === FSItemType.SpecialFolder)
          //special folder
          html +=
            '       <div class="silky-bs-flist-icon silky-bs-flist-item-folder-special-icon"></div>';
        else if (itemType === FSItemType.Drive)
          //drive
        ...
        if (item.description || item.tags || item.license) {
          ...
          html +=
            '       <div class="silky-bs-fslist-entry-name">' + name + "</div>";
          ...
          if (item.tags) {
            ...
            for (let tag of item.tags) {
              ...
              html += `<div class="tag" style="background-color: hsl(${hue}, 70%, 45%); border-color: hsl(${hue}, 70%, 45%);">${s6e(
                tag
              )}</div>`;
            }
            html += "</div>";
          ...
          html += `   <div class="silky-bs-fslist-entry-name">${s6e(
            name
          )}</div>`;
        }
      ...
      if (this.$items.length === 0) {
        this.clearSelection();
        this.$itemsList.append(
          `<span>${_("No recognised data files were found.")}</span>`
        );
      } else this._setSelection(this.$items[0]);
    }
  ...
  _setMultiMode: function (value) {
    if (value !== this.multiMode) {
      ...
      if (this.multiMode) {
        ...
        this.$footer
          .find(".silky-bs-fslist-import-options")
          .removeClass("hidden");
        $button.addClass("on");
      }
    }
  ...
  updateActiveDescendant() {
    if (this._selectedIndices.length > 0) {
      let $activeItem =
        this.$items[this._selectedIndices[this._selectedIndices.length - 1]];
      this.$itemsList.attr("aria-activedescendant", $activeItem.attr("id"));
    } else this.$itemsList.attr("aria-activedescendant", "");
  },
  removeNonFileItemsFromSelection() {
    let filtered = [];
    for (let i of this._selectedIndices) {
      let itemType = this.$items[i].data("type");
      if (itemType === FSItemType.File) filtered.push(i);
      else {
  ...
  clickItem: function (target, ctrlKey, shiftKey, fromChecked) {
    ...
    if (itemType !== FSItemType.File || this.model.clickProcess === "open") {
      ...
      itemType === FSItemType.File &&
      this.model.clickProcess === "import"
    ) {
      ...
      if (
        (multiSelect === false || modifier === false) &&
        this.multiMode === false
      ) {
          ...
          if (this._selectedIndices.length > 1) {
            ...
            for (let i = 1; i < this._selectedIndices.length; i++)
              name =
                name +
                ' "' +
                this.$items[this._selectedIndices[i]].data("name") +
                '"';
          }
        }
  ...
  incrementSelection: function () {
    ...
    if (selectedIndex !== -1 && selectedIndex !== this.$items.length - 1) {
      ...
      if (offset.top + height > this.$itemsList.height()) {
        let r =
          this.$itemsList.scrollTop() +
          (offset.top + height - this.$itemsList.height() + 1);
  ...
  _itemDoubleClicked: function (event) {
    ...
    let itemPath = $target.data("path");
    let itemTitle = $target.data("name");
    if (itemType === FSItemType.File) this._setMultiMode(false);
    if (itemType !== FSItemType.File || this.model.clickProcess === "open")
      this.model.requestOpen({
        path: itemPath,
        title: itemTitle,
  ...
  _nameChanged: function (event) {
    ...
    let name = this.$header
      ...
      .val()
      .trim();
    if (name === "") $button.addClass("disabled-div");
    else $button.removeClass("disabled-div");
  },
  _importNameChanged: function (event) {
    let $button = this.$footer.find(".silky-bs-fslist-browser-import-button");
    let name = this.$footer
      .find(".silky-bs-fslist-browser-import-name")
      .val()
      .trim();
    if (name === "") $button.addClass("disabled-div");
    else $button.removeClass("disabled-div");
  ...
  _hasValidExtension: function (name) {
    ...
    if (this.filterExtensions) {
      ...
      for (
        let extIndex = 0;
        extIndex < this.filterExtensions.length;
        extIndex++
      ) {
        if (
          name.toLowerCase().endsWith("." + this.filterExtensions[extIndex])
        ) {
          found = true;
          break;
        }
  ...
  _calcBackDirectory: function (filePath, type) {
    ...
    if (index <= 0) {
      if (this.model.attributes.wdType === "temp") return "{{Temp}}";

      if (this.model.attributes.wdType === "examples") return "{{Examples}}";
