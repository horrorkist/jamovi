...
const ResultsPanel = Backbone.View.extend({
  ...
  initialize(args) {
    ...
    ContextMenu.$el.on("menuClicked", (event, button) => {
      if (this._menuId !== null) this._menuEvent(button.eventData);
    });

    ContextMenu.on("menu-hidden", (event) => {
      if (this._menuId !== null) {
        if (this._menuId === 0) this._refsTable.deactivate();
        else this._menuEvent({ type: "activated", address: null });
        ...
        if (event !== undefined) {
          if (event.button === 2) {
            ...
            if (resource !== null)
              this._resultsMouseClicked(
                event.button,
                event.offsetX - resource.$container.offset().left,
                event.offsetY - resource.$container.offset().top,
                resource.analysis
              );
          }
        }
    ...
    if ("iframeUrl" in args) this.iframeUrl = args.iframeUrl;

    if ("mode" in args) this.mode = args.mode;
    ...
    this._refsTable.addEventListener("click", (event) =>
      this._resultsClicked(event, "refsTable")
    );
  ...
  _updateRefsMode() {
    ...
    if (refsMode === "bottom") this._refsTable.style.display = "";
    else this._refsTable.style.display = "none";
  },
  _checkIfEverythingReady() {
    if (this._ready) return;

    for (let id in this.resources) {
      if (!this.resources[id].sized) return;
    }
  ...
  _analysisCreated(analysis) {
    ...
    let classes = "";
    if (isEmptyAnalysis) classes = "empty-analysis";

    let $container = $(
      `<div class="jmv-results-container results-loop-list-item results-loop-auto-select  ${classes}" tabindex="-1" role="listitem" aria-label="${
        isEmptyAnalysis
          ? "Annotation Field"
    ...
    if (analysis.results.index > 0) {
      ...
      if (index < $siblings.length) $after = $siblings[index];
    }
    ...
    let $cover = $('<div class="jmv-results-cover"></div>').appendTo(
      $container
    );
    ...
    if (isEmptyAnalysis === false) {
      ...
      if (this.analysisCount === 1) $container.attr("tabindex", 0);
    } else {
  ...
  _analysisDeleted(analysis) {
    ...
    if (analysis.name === "empty") $container.remove();
    else {
      this.analysisCount -= 1;
      let removed = false;
      ...
      setTimeout(() => {
        if (removed === false) $container.remove();
      }, 400);
    }
  ...
  _updateRefs(exclude) {
    ...
    for (let analysis of this.model.analyses()) {
      if (analysis !== exclude && modulesWithRefChanges.includes(analysis.ns)) {
        ...
        if (res.loaded) this._sendRefNumbers(res);
      }
    }
  },
  _annotationEvent(sender, analysis, address) {
    if (sender !== this) {
      ...
      if (resources.loaded) this._sendResults(resources);
    }
  },
  _analysisNameChanged(analysis) {
    ...
    if (resources) {
      ...
      if (heading === null) heading = analysis.results.title;
  ...
  _sendResults(resources) {
    ...
    let event = {
      ...
      data: {
        ...
        annotationSelected:
          newSelected === null ? false : newSelected.name === "empty",
  ...
  _updateAll() {
    for (let id in this.resources) {
      ...
      if (resources.loaded === false) continue;
      this._sendResults(resources);
    }
  },
  _tryGetResource(xpos, ypos) {
    for (let id in this.resources) {
      ...
      if (
        xpos >= offset.left &&
        xpos <= offset.left + $container.width() &&
        ypos >= offset.top &&
        ypos <= offset.top + $container.height()
      )
  ...
  _resultsClicked(event, analysis) {
    ...
    if (analysis === "refsTable")
      ...
      this.model.set("selectedAnalysis", analysis);
    else this.model.set("selectedAnalysis", null);
  },
  ...
  _refsRightClicked(event) {
    ...
    if (nRefsSelected === 0) this._refsTable.activate();
  ...
  _messageEvent(event) {
    for (let id in this.resources) {
      let resources = this.resources[id];
      if (event.source !== resources.iframe.contentWindow) continue;
      ...
      switch (eventType) {
        ...
        case "annotationLostFocus":
          ...
          } else if (this._focus < 0) throw "shouldn't get here";
          break;
        ...
        case "sizeChanged":
          ...
          if ($iframe.height() === 0) $iframe.width(width);

          let selected = this.model.get("selectedAnalysis");
          if (
            eventData.scrollIntoView &&
            selected !== null &&
  ...
  getAsHTML(options, part) {
    if (!part) {
      ...
      let refs = formatIO
        .exportElem(this._refsTable, "text/html", options)
        .then((html) => {
          return { html };
        });
      promises.push(refs);
    ...
    return this._getContent(address, options).then((content) => content.html);
  },
  _getContent(address, options) {
    ...
    iframeWindow.postMessage(
      { type: "getcontent", data: { address, options } },
      "*"
    );
  ...
  async _menuEvent(event) {
    if (event.op === "copy") {
      ...
      this._getContent(event.address, options)
        .then((content) => {
      ...
      iframeWindow.postMessage(
        { type: "addNote", data: { address, options } },
        "*"
      );
      ...
      if (event.target.type === "Image") {
        let options = {
          ...
          filters: [
            ...
            {
              name: "PNG",
              description: _("PNG Image {ext}", { ext: "(.png)" }),
              extensions: ["png"],
            },
            {
              name: "SVG",
              description: _("SVG Image {ext}", { ext: "(.svg)" }),
              extensions: ["svg"],
            },
            {
              name: "EPS",
              description: _("EPS Image {ext}", { ext: "(.eps)" }),
              extensions: ["eps"],
            },
          ],
        };
        ...
        if (host.isElectron === false && status.path) {
          ...
          let url = `dl/${source}?filename=${encodeURIComponent(
        ...
        if (part === "")
          options.filters.push({
            name: _("LaTeX bundle {ext}", { ext: "(.zip)" }),
            extensions: ["zip"],
          });
      ...
      if (this._menuId > 0 && this._menuId in this.resources) {
        ...
        this.resources[this._menuId].iframe.contentWindow.postMessage(
          message,
          this.iframeUrl
        );
      }
  ...
  setFocus(analysis) {
    if (!analysis) {
      let analysisObjs = Array.from(this.model.analyses()).filter((analysis) =>
        analysis.hasUserOptions()
      );
      if (analysisObjs.length > 0) analysis = analysisObjs[0];
    }
    focusLoop.enterFocusLoop(this.el, { withMouse: false });
    if (analysis) {
      let newSelectedResults = this.resources[analysis.id];
      if (newSelectedResults)
        this.resultsLooper.selectElement(
          newSelectedResults.$container[0],
          false
        );
    }
  },
  ...
  _sendSelected(resourceId) {
    ...
    for (let id in this.resources) {
      ...
      if (selectedResource)
        event.data.annotationSelected =
          selectedResource.analysis.name === "empty";
  ...
  annotationLostFocus() {
    ...
    } else if (this.annotationFocus < 0) throw "shouldn't get here";
  },
  _updateEditState() {
    let event = {
      type: "annotationEvent",
